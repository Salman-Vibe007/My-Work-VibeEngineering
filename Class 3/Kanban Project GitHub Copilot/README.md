...
This repository contains the deployable Next.js app in `frontend/`.

1. Import the repository into Vercel.
2. Set the Vercel **Root Directory** to `Kanban Project GitHub Copilot/frontend`.
3. Keep the framework as **Next.js** and deploy. The included `vercel.json` uses `npm ci` and `npm run build`.

No environment variables are required. The board is client-side only and does not use an API or database.
